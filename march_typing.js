...
function check_mobile()
{
    ...
    if(screen.width<800)
    {
        ...
        // document.querySelector(".welcome").innerHTML="<h1>Please use a laptop/desktop</h1><h2>Not Ready for phones</h2>";
        return true;
    }
    return false;
}
...
function processText()          //newly added in March to add mobile compatability
{
...
document.addEventListener('mykeydown',function(){

  console.log("MYkeydown eventListener called "+ event.keycode);
                  //all this inside a callback

  if(!gameStarted)
  return;

  if(j==quotes_array[quoteNo].length)
  {
    j=0;
    updateQuote();
  ...
  let eventkey = String.fromCharCode(event.keycode);     //new addition

  if(!((event.keycode>=65 && event.keycode<=90) || (event.keycode>=97 && event.keycode<=122)|| (event.keycode==' ') || (eventkey=='.') || (eventkey==',') || (eventkey==';') || (eventkey=="'")))
  return;
  ...
  if(quotes_array[quoteNo][j]==eventkey)
  {
...
document.addEventListener('keydown',function(){
  ...
  console.log("keydown eventListener called "+ event.keyCode);
                  //all this inside a callback
